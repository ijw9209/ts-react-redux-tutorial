//Ducks 패턴 , 액션타입, 액션생성함수, 리듀서를 모두 한 파일에

// 액션 타입 선언
// 뒤에 as const 를 붙임으로 나중에 액션 객체를 만들게 action.type의 값을 추론하는 과정에서
...
export const increaseBy = (diff: number) => ({
    ...
    //이는 FSA (https://github.com/redux-utilities/flux-standard-action) 라는 규칙
...
// ReturnType<typeof ______> 는 특정 함수의 반환값을 추론
// 상단부에서 액션 타입을 선언 할 때 as const를 하지 않으면 이 부분이 작동하지 않음
type CounterAction =
